...
MYINFO="'编译日期 $valtime by Tekin(https://github.com/cxsw/openwrt-rom),云南网站研发中心(https://dev.yunnan.ws)'"
echo $MYINFO
#自定义lan ip, 设置中国时区
...
# 附件中的4个升级包中带有 k2, k2p, x86-64, x86 根据你的设备型号和类型选择下载一个即可

1. 名字中有22.03的是**稳定版[22.03分支]**,偶尔会有更新
2. 没有22.03的是**最新开发版[master分支]**,每天都会有不少更新
3. 安装后可以从路由器内从软件包内更新,不用每次刷整包
4. 刷机教程见 https://blog.csdn.net/tekin_cn/article/details/135484994
